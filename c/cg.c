...
// Flag to say which section are outputting in to.
...
// We store offset as positive to make aligning the stack pointer easier.
static int localOffset;
static int stackOffset;

// Create the position of a new local vairable.
...
// The lost also includes the registers usedto
...
// Check to see if it's not alread there.
...
// Return the number of the reigster.
// For x86-64, we don't need to worry about the type.
int cgloadint(int value, int type) {
    // Get a new register
    int r = alloc_register();
    // Printout the code to initialise it
...
// Divide the first register by second and
// return the number of the register with the result.
...
// Jump if it's an IF or WHILE operation.
...
void cgprintint(int r) {
    ...
    fprintf(Outfile, "\tcall\t_printint\n");
    free_register(r);
}

// Call a function with the given symbol id.
// Pop off any argument pushed on the stack.
...
void cgcopyarg(int r, int argposn) {
    ...
    // successive argument in the correct order for x86-64.
    if (argposn > 6) {
...
// List of inverted jump instruction.
// in AST order: A_EQ, A_NE, A_LT, A_GT, A_LE, A_GE
...
// Store through a dereference pointer.
