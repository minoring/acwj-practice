...
// i.e. whitespaces, newlines. Return the first
...
static int scanch(void) {
    ...
    // metacharacters that starts with a backslash.
    c = next();
    if (c == '\\') {
        switch (c = next()) {
        case 'a':
...
static int scanident(int c, char *buf, int lim) {
    ...
    // We hit a non-valid characer, put it back.
    putback(c);
    buf[i] = '\0';
    return (i);
}
...
// Return 1 if token valid, 0 if n tokens left.
