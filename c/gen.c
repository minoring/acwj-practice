...
static int genIF(struct ASTnode *n) {
    ...
    // When there is no ELSE clause, Lfalse is
    // the endling label.
    Lfalse = label();
    if (n->right) {
        Lend = label();
    ...
    // generate the jump to skip the end
    if (n->right) {
        cgjump(Lend);
    }
    // Now the false label
...
int genAST(struct ASTnode *n, int reg, int parentASTop) {
    ...
    // We now have specific AST node handling at the op
    switch (n->op) {
    case A_IF:
        return (genIF(n));
    case A_WHILE:
    ...
    case A_GE:
        ...
        // followed by a jump. Otherwise, compare a registers and
